...
get_nodes() {
   ...
   translated_rule="$(curl -s -X POST --cert $puppet_host_cert --key $puppet_host_key --cacert $puppet_local_cacert \
      "https://$puppet_server:4433/classifier-api/v1/rules/translate?format=inventory" -H 'Content-Type: application/json' --data "$rule" | \
      jq -c '.query')"
   ...
   if [[ $rule == "null" ]]; then
      ...
      nodes=($(curl -s --cert $puppet_host_cert --key $puppet_host_key --cacert $puppet_local_cacert -G \
         "https://$puppet_server:8081/pdb/query/v4/inventory" --data-urlencode "query=[\"extract\", \"certname\", $translated_rule]" | jq -rc '.[] | .certname'))
   fi
...
puppet_local_cacert="$(puppet config print localcacert)"
...
if (( $# > 0 )); then
   mapfile -t lines < <(curl -s https://$puppet_server:4433/classifier-api/v1/groups --cert \
      $puppet_host_cert --key $puppet_host_key --cacert $puppet_local_cacert | jq -rc ".[] | select(.name == \"$1\") | .name, .rule"
   )
   (( ${#lines[@]} > 0 )) || {
   ...
   mapfile -t lines < <(curl -s https://$puppet_server:4433/classifier-api/v1/groups --cert \
      $puppet_host_cert --key $puppet_host_key --cacert $puppet_local_cacert | jq -rc ".[] | .name, .rule"
   )
   (( ${#lines[@]} > 0 )) || {
